...
public class ReceiveSppDataThread extends Thread {
    ...
    public ReceiveSppDataThread(BluetoothDevice device, UUID sppUUID, BluetoothSocket socket, OnRecvSppDataListener listener) {
        this(device, sppUUID, socket, 4096, listener);
    }

    public ReceiveSppDataThread(BluetoothDevice device, UUID sppUUID, BluetoothSocket socket, int blockSize, OnRecvSppDataListener listener) {
        ...
        if (mConnectedSppDev != null) {
            ...
            while (isRunning && null != inputStream) {
                try {
                    ...
                    JL_Log.e(TAG, "-ReceiveDataThread- have an exception : " + e.toString() + ", sppUUID = " + mSppUUID);
                    e.printStackTrace();
